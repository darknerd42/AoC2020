class Day18: DayInterface {
    ...
    private fun solvePart1(input: List<String>): Long? {
        val values = input.map { evaluateExpression(it) }

        return values.sum()
    }

    private fun solvePart2(input: List<String>): Long? {
        val values = input.map { evaluateAdvancedExpression(it) }
    ...
    private fun evaluateExpression(e: String): Long {
        var parenIndex = findParen(e)
        var currentExpression = e

        while (parenIndex != null) {
            val inner = evaluateExpression(currentExpression.substring(parenIndex.first+1, parenIndex.second))
            currentExpression = currentExpression.replaceRange(parenIndex.first..parenIndex.second, inner.toString())
            parenIndex = findParen(currentExpression)
        }

        val elements = currentExpression.split(" ")
        var acc = 0L
        var nextOp = Operator.START
        for (element in elements) {
            when (element) {
                "+" -> nextOp = Operator.ADD
                "*" -> nextOp = Operator.MULT
                else -> {
                    when (nextOp) {
                        Operator.START -> acc = element.toLong()
                        Operator.MULT -> acc *= element.toLong()
                        Operator.ADD -> acc += element.toLong()
                    }
    ...
    private fun evaluateAdvancedExpression(e: String): Long {
        var parenIndex = findParen(e)
        var currentExpression = e

        while (parenIndex != null) {
            val inner = evaluateAdvancedExpression(currentExpression.substring(parenIndex.first+1, parenIndex.second))
            currentExpression = currentExpression.replaceRange(parenIndex.first..parenIndex.second, inner.toString())
            parenIndex = findParen(currentExpression)
        }

        val elements = currentExpression.split(" ").toMutableList()
        var acc = 0L
        var nextOp = Operator.START
        var plusIndex = elements.indexOf("+")

        // Evaluate all the + first:
        while (plusIndex != -1) {
            val sum = elements[plusIndex-1].toLong() + elements[plusIndex+1].toLong()
            elements[plusIndex-1] = sum.toString()
            elements.removeAt(plusIndex) // removes +
            elements.removeAt(plusIndex) // removes next number

            plusIndex = elements.indexOf("+")
        }
    ...
    private fun findParen(s: String): Pair<Int, Int>? {
        val start = s.indexOf('(')
        if (start == -1) return null
        var openCount = 1
        for (i in (start+1)..s.lastIndex) {
            when (s[i]) {
                '(' -> openCount++
                ')' -> openCount--
            }
            if (openCount == 0) return Pair(start, i)
        }

        error("Could not find closing paren")
    }

typealias RuleList = Map<String, List<Day7.ContentRule>>

class Day7: DayInterface {

    public override fun part1Test(): Long? {
        return part1Answer(testInput, "shiny gold")
    }
    public override fun part1Answer(): Long? {
        return part1Answer(puzzleInput, "shiny gold")
    }
    public override fun part2Test(): Long? {
        return part2Answer(testInput, "shiny gold")
    }
    public override fun part2Answer(): Long? {
        return part2Answer(puzzleInput, "shiny gold")
    }

    data class ContentRule(val quantity: Int, val color: String) {}

    class Bag(color: String, rules: RuleList) {
        public val contents: MutableList<Content> = mutableListOf()
        public val color: String = color

        data class Content(val quantity:Int, val bag: Bag) {}

        init {
            val contentRules = rules[color]

            if (contentRules != null && !contentRules.isEmpty()) {
                for (bag in contentRules) {
                    contents.add(Content(bag.quantity, Bag(bag.color, rules)))
                }
            }
        }

        public fun contains(whichBag: String): Boolean {
            if (contents.map { it.bag.color }.contains(whichBag)) return true
            for (content in contents) {
                if (content.bag.contains(whichBag)) return true
            }
            return false
        }

        public fun numberContainedBags(): Int {
            if (contents.isEmpty()) return 0

            return contents.sumBy {
                it.quantity * (it.bag.numberContainedBags() + 1)
            }
        }
    }

    private fun parseRules(input: List<String>): RuleList {
        var rules = mutableMapOf<String, List<ContentRule>>()

        for (rule in input) {
            val bagColorMatchResult = "([a-z ]+) bags contain ".toRegex().find(rule)

            val ruleMatches = "([0-9]+) ([a-z ]+) bag[s]?".toRegex().findAll(rule)

            if (bagColorMatchResult == null) continue

            if ("no other bags".toRegex().find(rule) != null) {
                rules.put(bagColorMatchResult.groupValues[1], listOf<ContentRule>())
            } else if (ruleMatches.count() < 1) {
                continue
            } else {
                rules.put(bagColorMatchResult.groupValues[1], ruleMatches.toList().map {
                    ContentRule(it.groupValues[1].toInt(), it.groupValues[2])
                })
            }
        }
    ...
    private fun part1Answer(input: List<String>, whichBag: String): Long? {
        val rules = parseRules(input)
        var count = 0L
        var bags = mutableListOf<Bag>()

        for (rule in rules) {
            bags.add(Bag(rule.key, rules))
        }

        for (bag in bags) {
            if (bag.contains(whichBag)) count++
        }

        return count
    }

    private fun part2Answer(input: List<String>, whichBag: String): Long? {
        val rules = parseRules(input)
        var count = 0L
        var bags = mutableListOf<Bag>()
        ...
        val containedBags = bags.map { it.numberContainedBags() }

        val bag = bags.find { it.color == whichBag }

        return bag?.numberContainedBags()?.toLong()
    }

class Day2: DayInterface {
    ...
    private data class PasswordLine(val first: Int, val second: Int, val requiredCharacter: Char, val password: String) {
    }

    public override fun part1Test(): Long? {
        return getAnswer(testInput, part1Validator)?.toLong()
    }
    public override fun part2Test(): Long? {
        return getAnswer(testInput, part2Validator)?.toLong()
    }
    public override fun part1Answer(): Long? {
        return getAnswer(puzzleInput, part1Validator)?.toLong()
    }
    public override fun part2Answer(): Long? {
        return getAnswer(puzzleInput, part2Validator)?.toLong()
    }

    private fun getAnswer(input: List<String>, validator: (PasswordLine)-> Boolean): Int? {
        var passwordLines = mutableListOf<PasswordLine>()
        var numberOfInvalidPasswords = 0

        input.forEach{
            val line = parseString(it)
            if (line != null) passwordLines.add(line)
        }

        passwordLines.forEach{
            if (!validator(it)) {
                numberOfInvalidPasswords++
            }
        }

        val totalPasswords = passwordLines.count()
        return (totalPasswords - numberOfInvalidPasswords)
    }


    private val part1Validator: (PasswordLine) -> Boolean = { line: PasswordLine -> checkPassword(line) }
    private val part2Validator: (PasswordLine) -> Boolean = { line: PasswordLine -> checkPasswordAgainstNewPolicy(line) }

    private fun parseString(input: String): PasswordLine? {
        val regex = Regex("([0-9]+)-([0-9]+) ([a-z]+): (.+)")
        val matchingGroup = regex.find(input)

        if (matchingGroup != null) {
            return PasswordLine(
                matchingGroup.groupValues[1].toInt(),
                matchingGroup.groupValues[2].toInt(),
                matchingGroup.groupValues[3][0],
                matchingGroup.groupValues[4]
            )
        }
    ...
    private fun checkPassword(line: PasswordLine): Boolean {
        val count = line.password.filter{ it == line.requiredCharacter }.count()
        return count in line.first..line.second
    }

    private fun checkPasswordAgainstNewPolicy(line: PasswordLine): Boolean {
        return (line.password[line.first-1] == line.requiredCharacter).xor(line.password[line.second-1] == line.requiredCharacter)
    }
}

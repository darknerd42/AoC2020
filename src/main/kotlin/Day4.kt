...
"""
    ...
    data class Passport(
        ...
        public fun isValidPart2(): Boolean {
            return isValid_byr() &&
                   isValid_iyr() &&
                   isValid_eyr() &&
                   isValid_hgt() &&
        ...
        private fun isValid_byr(): Boolean {
            if (byr == null) return false
            if (byr.count() != 4) return false
            val value = byr.toInt()
            return value in 1920..2002
        }
        private fun isValid_iyr(): Boolean {
            if (iyr == null) return false
            if (iyr.count() != 4) return false
            val value = iyr.toInt()
            return value in 2010..2020
        }
        private fun isValid_eyr(): Boolean {
            if (eyr == null) return false
            if (eyr.count() != 4) return false
            val value = eyr.toInt()
            return value in 2020..2030
        }
        private fun isValid_hgt(): Boolean {
            if (hgt == null) return false
            val regex = Regex("([0-9]+)(cm|in)")
            val matches = regex.find(hgt) ?: return false
            val (height, unit) = matches.destructured
        ...
        private fun isValid_hcl(): Boolean {
            if (hcl == null) return false
            return "#[0-9a-f]{6}".toRegex().matches(hcl)
        }
        private fun isValid_ecl(): Boolean {
            return when (ecl) {
        ...
        private fun isValid_pid(): Boolean {
            if (pid == null) return false
            return "[0-9]{9}".toRegex().matches(pid)
        }
    }

    private fun separatePassports(input: String): List<String> {
        var listOfPassportInputs = mutableListOf<String>("")

        for (line in input.lines()) {
            if (line == "") {
                listOfPassportInputs.add("")
            }
            else
            {
                listOfPassportInputs[listOfPassportInputs.lastIndex] += " $line"
            }
        }

        listOfPassportInputs.removeIf { it == "" }
        return listOfPassportInputs
    }

    private fun parsePassport(input: String): Passport {
        val parameterStrings = input.split(" ")
        var byr: String? = null
        var iyr: String? = null
        var eyr: String? = null
        var hgt: String? = null
    ...
    private fun countValid(passports: List<Passport>): Long {
        return passports
            .map { if (it.isValid()) 1L else 0L }
            .reduce { sum, validCount -> sum + validCount }
    }

    private fun countValidPart2(passports: List<Passport>): Long {
        return passports
            .map { if (it.isValidPart2()) 1L else 0L }
            .reduce { sum, validCount -> sum + validCount }
    }

    public override fun part1Test(): Long? {
        var passportInputs = separatePassports(testInput)
        var passports = passportInputs.map { parsePassport(it) }
        return countValid(passports)
    }
    public override fun part1Answer(): Long? {
        var passportInputs = separatePassports(puzzleInput)
        var passports = passportInputs.map { parsePassport(it) }
        return countValid(passports)
    }
    public override fun part2Test(): Long? {
        var passportInputs = separatePassports(testInput2)
        var passports = passportInputs.map { parsePassport(it) }
        return countValidPart2(passports)
    }
